module ApplicationHelper
  def display_buttons
    buttons = []
    if logged_in?
      buttons.push(link_to(current_user.username, user_path(current_user), class: 'text-light mr-2 bg-success py-1 px-3'))
  ...
  def user_attending?(event_id)
    ...
    if events_created.find_by(id: :event_id)
      ...
      buttons.push(link_to('Enroll', attendances_path(controller: 'invites', action: 'create', event_id: event_id), method: :post, class: 'btn btn-outline-success'))
      buttons.push(link_to('Cancel', attendance_path(event_id), method: :delete, class: 'btn btn-outline-danger mt-1'))
    end
    buttons
  end
